...
用户输入一段素材描述后，Pixel Asset Forge 会解析提示词中的语义线索，并将其拆分为主体、颜色、材质、环境和特效层。例如“冰霜法师”会触发冷色调、魔法职业和披风细节，“火焰水晶剑”会触发刀剑主体、水晶材质和火焰符文特效。生成逻辑由确定性 seed 驱动，同一组输入可以复现同一结果。
...
- 道具：适合剑、斧、矛、锤、弓、箭、盾牌、钥匙、卷轴、书、宝箱、戒指、炸弹、药水、金币、水晶等可拾取或装备物品。
- 地块：适合石板、水面、沙地、木板、金属地面、森林苔藓、毒液、熔岩等地图 tile、地表纹理和关卡原型。
...
- 主体词优先：角色、道具、地块和 UI 图标分别使用不同的主体优先级，剑、斧、矛、锤、弓、钥匙、宝箱、卷轴、金币、药水等实体词会优先决定主体，魔法、符文、火焰、冰霜、毒液等词作为材质和特效层叠加。
- 方向性高光：通过亮部和暗部提升体积感。
